...
/**
 * Day3 실습 — 감사 로그. 가장 바깥(order 0)에 둬서, 이후 어떤 Advisor 가 요청을 바꾸거나
 * 막아도(예: {@link SafetyAdvisor} 의 거절) "요청이 들어왔다"는 사실만은 항상 남긴다.
 *
 * <p>{@link BaseAdvisor} 하나만 구현하면 {@code call}·{@code stream} 양쪽을 다 커버한다 —
 * {@code CallAdvisor} 만 구현하면 스트리밍 응답에서 감사가 누락된다.
 */
public class AuditAdvisor implements BaseAdvisor {
    ...
    public AuditAdvisor(Lab3AuditLog auditLog) {
        this.auditLog = auditLog;
    }

    @Override
    public int getOrder() {
        return Ordered.HIGHEST_PRECEDENCE; // 가장 바깥
    }
